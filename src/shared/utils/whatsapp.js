...
import { rawDigits } from './formatters.js'

export function whatsappLink(phone, phoneCountry, message) {
  const digits = rawDigits(phone || '')
  if (!digits) return null
  const dial = phoneCountry || '55'
  const msg = encodeURIComponent(message || 'Olá! Aqui é da Baruk Technology.')
